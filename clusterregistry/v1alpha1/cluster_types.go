...
func (c *Cluster) ContainerRuntimeVersion(baseDomain string) string {
	if c.Annotations == nil {
		return ""
	}
	key := fmt.Sprintf("%s/container-runtime-version", baseDomain)
	return c.Annotations[key]
}

func (c *Cluster) CNI(baseDomain string) string {
	if c.Annotations == nil {
		return ""
	}
	key := fmt.Sprintf("%s/network-type", baseDomain)
	return c.Annotations[key]
}
...
func (c *Cluster) IsAmd64Arch(baseDomain string) bool {
	if c.Annotations == nil {
		return false
	}
	key := fmt.Sprintf("%s/arch-amd64", baseDomain)
	if v, exist := c.Annotations[key]; exist && v == "true" {
		return true
	}
	return false
}

func (c *Cluster) IsArm64Arch(baseDomain string) bool {
	if c.Annotations == nil {
		return false
	}
	key := fmt.Sprintf("%s/arch-arm64", baseDomain)
	if v, exist := c.Annotations[key]; exist && v == "true" {
		return true
	}
	return false
...
func (c *Cluster) IsMultiArch(baseDomain string) bool {
	if c.Annotations == nil {
		return false
	}
	arches := 0
...
func (c *Cluster) IsSupportIPv4(baseDomain string) bool {
	if c.Labels == nil {
		return false
	}
	key := fmt.Sprintf("%s/protocol-stack", baseDomain)
	if v, exist := c.Labels[key]; exist {
		return v == "Dual-Stack" || v == "IPv4"
	}
	return false
}

func (c *Cluster) IsSupportIPv6(baseDomain string) bool {
	if c.Labels == nil {
		return false
	}
	key := fmt.Sprintf("%s/protocol-stack", baseDomain)
	if v, exist := c.Labels[key]; exist {
		return v == "Dual-Stack" || v == "IPv6"
	}
	return false
}

func (c *Cluster) IsSupportDualStack(baseDomain string) bool {
	if c.Labels == nil {
		return false
	}
	key := fmt.Sprintf("%s/protocol-stack", baseDomain)
...
func (c *Cluster) HasControlPlane(baseDomain string) bool {
	if c.Annotations == nil {
		return false
	}
	key := fmt.Sprintf("%s/nodes-mode", baseDomain)
	if v, exist := c.Annotations[key]; exist && v == "self-managed" {
		return true
	}
	return false
}

func (c *Cluster) CloudProvider(baseDomain string) string {
	if c.Annotations == nil {
		return ""
	}
	key := fmt.Sprintf("%s/cloud-provider", baseDomain)
	return c.Annotations[key]
}

func (c *Cluster) ImportedClusterWarning(baseDomain string) string {
	if c.Annotations == nil {
		return ""
	}
	key := fmt.Sprintf("%s/imported-cluster-warning", baseDomain)
	return c.Annotations[key]
}
